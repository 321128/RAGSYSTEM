# Retrieval Debugging Guide for Medical RAG
...
curl -s -X POST http://127.0.0.1:5200/debug/retrieve-mmr \
  -H 'Content-Type: application/json' \
  -d '{"question":"What is diabetes?","lambda_mult":0.5}'
```
...
curl -s -X POST http://127.0.0.1:5200/debug/retrieve-threshold \
  -H 'Content-Type: application/json' \
  -d '{"question":"What is diabetes?","threshold":0.5}'
```
...
- Embedding model not trained on medical terminology
- Questions too vague or abstract

**Solutions:**
1. **Try a medical-specific embedding model:**
   ```env
...
2. **Descriptive**: "Explain the pathophysiology of type 2 diabetes"
   - Should return comprehensive mechanism explanation

3. **Comparative**: "Compare type 1 and type 2 diabetes"
...
| Issue | Cause | Fix |
|-------|-------|-----|
| All similarity scores < 0.3 | Wrong embedding model or vocabulary mismatch | Try medical-specific embeddings or re-chunk |
