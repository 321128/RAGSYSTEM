# Agent Prompt — Medical RAG Offline Desktop Application
...
Build a **fully offline, self-contained medical document question-answering system** that can be distributed via USB pendrive to end users running Ubuntu. The system uses Retrieval-Augmented Generation (RAG) — users upload medical PDF documents, the system ingests and indexes them, and users can ask plain-language questions and receive answers grounded only in those documents.
...
STEP 2 — Build prompt
  ...
  SYSTEM:
  You are a medical document assistant. Answer questions using ONLY the
...
3. If retrieved_count == 0 → return `{ "answer": "No relevant documents found. Please upload medical PDFs first.", "sources": [], "debug": {...} }`
4. Generate answer
...
echo "========================================"
echo "  MyApp Medical RAG — Installer"
echo "========================================"
echo ""
...
# Desktop shortcut
DESKTOP_FILE="$HOME/Desktop/MedicalRAG.desktop"
cat > "$DESKTOP_FILE" << EOF
[Desktop Entry]
Version=1.0
Name=Medical RAG
Comment=Medical Document Q&A System
...
- [ ] A user can upload a medical PDF and see status change from pending → processing → completed
