# End-to-End Medical Document RAG System using LangChain + LangSmith

## Goal
Build a replacement for RAGFlow that:
- Parses medical PDFs, scanned reports, prescriptions, discharge summaries, lab reports, and guidelines
...
For medical use cases, PostgreSQL + pgvector is usually better than Chroma because:
- easier backup
- persistent
- production ready
- SQL filtering on patient/document metadata
...
services:
  postgres:
    ...
    container_name: medical_pgvector
    restart: always
    environment:
      POSTGRES_USER: admin
...
```bash
docker exec -it medical_pgvector psql -U admin -d medicalrag
```
...
DATABASE_URL=postgresql://admin:admin123@localhost:5432/medicalrag
```
...
Medical documents usually contain:
- tables
- scanned handwriting
- headers/footers
- lab values
...
For medical documents do NOT use fixed 1000-character chunks only.
...
This reduces hallucination and improves accuracy on medical questions.
...
9. Add human review workflow for medical answers
